...
public class BookingServiceImpl implements BookingService {
    ...
    public Collection<Booking> getBookingsByState(GetBookingsDto dto) {
        ...
        return switch (dto.getState()) {
            case ALL -> rep.getBookingsByStateAll(dto.getUserId());
            case CURRENT -> rep.getBookingsByStateCurrent(dto.getUserId(), LocalDateTime.now());
            case PAST -> rep.getBookingsByStatePast(dto.getUserId(), LocalDateTime.now());
            case FUTURE -> rep.getBookingsByStateFuture(dto.getUserId(), LocalDateTime.now());
            case WAITING -> rep.getBookingsByStateWaiting(dto.getUserId());
            case REJECTED -> rep.getBookingsByStateRejected(dto.getUserId());
    ...
    public Collection<Booking> getBookingsByOwner(GetBookingsDto dto) {
        userService.contains(dto.getUserId());
        return switch (dto.getState()) {
            case ALL -> rep.getBookingsOwnerByStateAll(dto.getUserId());
            case CURRENT -> rep.getBookingsOwnerByStateCurrent(dto.getUserId(), LocalDateTime.now());
            case PAST -> rep.getBookingsOwnerByStatePast(dto.getUserId(), LocalDateTime.now());
            case FUTURE -> rep.getBookingsOwnerByStateFuture(dto.getUserId(), LocalDateTime.now());
            case WAITING -> rep.getBookingsOwnerByStateWaiting(dto.getUserId());
            case REJECTED -> rep.getBookingsOwnerByStateRejected(dto.getUserId());
            default -> throw new ObjectNotFoundException("Не существует такого состояния бронирования");
        };
    }
