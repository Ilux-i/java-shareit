...
public class ItemServiceImpl implements ItemService {
    ...
    public Item add(NewItemDto dto) {
        ...
        ItemRequest request = requestService.getOne(dto.getRequestId());
        Item item = ItemMapper.toItem(dto, owner, request);
        validOwner(item.getOwner().getId());
    ...
    public Item update(UpdateItemDto dto) {
        Item oldItem = rep.findById(dto.getId())
                .orElseThrow(() -> new ObjectNotFoundException("Item с id: " + dto.getId() + " не найден"));
    ...
    private Item updated(Item oldItem, UpdateItemDto dto) {
        ...
        if (dto.getRequest() == null)
            ...
            throw new ObjectNotFoundException("Попытка обновить Request в Item");
